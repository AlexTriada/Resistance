...
if !(alive _cured) exitWith
	{
	if (_player) then {hint format ["%1 мёртв",name _cured]};
...
if ((not("ACE_epinephrine" in (items _medicX))) and (not("ACE_epinephrine" in (items _cured)))) exitWith {
	...
	if (_player) then {hint format ["У Вас или у %1 должен быть морфин, чтобы его оживить",name _cured]};
...
if ((not("ACE_epinephrine" in (items _medicX))) and !(_medicX canAdd "ACE_epinephrine")) exitWith
	{
...
if !(_cured getVariable ["INCAPACITATED",false]) exitWith
	{
	if (_player) then {hint format ["%1 больше не нужна ваша помощь",name _cured]};
...
if (not("ACE_epinephrine" in (items _medicX))) then
	{
...
if (!_player) then
	...
	{
	_actionX = _medicX addAction ["Отменить оживление", {(_this select 1) setVariable ["cancelRevive",true]},nil,6,true,true,"","(_this getVariable [""helping"",false]) and (isPlayer _this)"];
	};
...
if !(alive _cured) exitWith
	{
	if (_player) then {hint format ["Мы потеряли %1",name _cured]};
...
if (_medicX getVariable ["success",true]) then
	...
	{
	if (_player) then {hint "Оживление успешно"};
	if (_inPlayerGroup) then {_medicX groupChat "Оживление неудачно"};
	};
