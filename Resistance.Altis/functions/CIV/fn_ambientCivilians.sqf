/*
	Depending: maxUnits, trafficRate, arrayCivs, listMilBld, Base_fnc_createUnit, Init_fnc_initCivilian, UPSMON.sqf
*/

#define CIV_DISTANCE 300
#define SLEEP_TIME 5

private _getAliveCivilians =
{
	allUnits select { alive _x && { side _x == civilian }}
};

private _isFarFromObjects =
{
...
private _deleteAwayCivilians =
{
	...
	{
		if ([_x, _humanPlayers, CIV_DISTANCE] call _isFarFromObjects) then
		{
			deleteVehicle _x;
		};
...
while { true } do
{
	call _deleteAwayCivilians;
	call _addRandomCivilians;
