...
[_vehicleType] call RES_fnc_displayVehiclePlacementMessage;
...
addMissionEventHandler ["EachFrame",
	...
	if (vehPlace_actionToAttempt != VEHPLACE_NO_ACTION) then
		...
		switch(vehPlace_actionToAttempt) do
			...
			case VEHPLACE_ACTION_PLACE:
				{
					[] spawn RES_fnc_attemptPlaceVehicle;
					_shouldExitHandler = true;
				};
			case VEHPLACE_ACTION_EXIT:
				{
					[] spawn RES_fnc_handleVehPlacementCancelled;
					_shouldExitHandler = true;
				};
			case VEHPLACE_ACTION_RELOAD:
				{
					...
					vehPlace_previewVeh allowDamage false;
					vehPlace_previewVeh enableSimulation false;
					[_typeX] call RES_fnc_displayVehiclePlacementMessage;
				};
	...
	if (!_shouldExitHandler) then {
		private _shouldCancelArray = [vehPlace_callbackTarget, CALLBACK_SHOULD_CANCEL_PLACEMENT, [vehPlace_previewVeh]] call RES_fnc_vehPlacementCallbacks;
	...
	private _isValidLocationArray = [vehPlace_callbackTarget, CALLBACK_VEH_IS_VALID_LOCATION, [_placementPos, getDir vehPlace_previewVeh, typeOf vehPlace_previewVeh]] call RES_fnc_vehPlacementCallbacks;
