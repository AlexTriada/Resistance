...

{
	...
	if (_x == _x getVariable ["owner", _x] && isPlayer _x) then {
		if ([_x] call RES_fnc_isMember) then
		{
			...
			if (_x getVariable ["eligible",true]) then
			{
				...
				{
					_LeaderX = _x;
					_dataX = [_LeaderX] call RES_fnc_numericRank;
...
_promoted = false;
{
	...
	_dataX = [_x] call RES_fnc_numericRank;
	...
	if (_rank != "COLONEL") then
	{
		...
		{
			_promoted = true;
			[_x,_newRank] remoteExec ["RES_fnc_ranksMP"];
			_x setVariable ["rankX",_newRank,true];
			_textX = format ["%3%1: %2.\n",name _x,_newRank,_textX];
			[-1*(50*_multiplier),_x] call RES_fnc_playerScoreAdd;
...
if (_promoted) then
{
	...
	[petros,"hint",_textX] remoteExec ["RES_fnc_commsMP"];
};
...
if (!isNull _suggestedLeader && {_suggestedLeader in _eligibles}) then
{
	...
	{
		_dataX = [_suggestedLeader] call RES_fnc_numericRank;
...
if (!isNull _selectable) then
{
	...
	[_selectable] call RES_fnc_theBossInit;
	sleep 5;
	[[petros,"hint",_textX],"RES_fnc_commsMP"] call BIS_fnc_MP;
};
