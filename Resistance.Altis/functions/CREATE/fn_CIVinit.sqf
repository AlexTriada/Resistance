...
[
	"killed",
	{
		...
		{
			_nul = [-1,-1,getPos _victim] remoteExec ["RES_fnc_citySupportChange",2];
		}
		else
		{
			if (isPlayer _killer) then
			{
				if (typeOf _victim == "C_man_w_workeRES_F") then {_killer addRating 1000};
				[-10,_killer] call RES_fnc_playerScoreAdd;
				//[_killer,20,0.34] remoteExec ["RES_fnc_punishment",_killer];
			};
			_multiplier = 1;
			...
			{
				_nul = [1*_multiplier,0] remoteExec ["RES_fnc_prestige",2];
				_nul = [1,0,getPos _victim] remoteExec ["RES_fnc_citySupportChange",2];
			}
			else
			{
				...
				{
					//_nul = [-1*_multiplier,0] remoteExec ["RES_fnc_prestige",2];
					_nul = [0,1,getPos _victim] remoteExec ["RES_fnc_citySupportChange",2];
				}
				else
				{
					if (side group _killer == Invaders) then
					{
						//_nul = [2*_multiplier,0] remoteExec ["RES_fnc_prestige",2];
						_nul = [-1,1,getPos _victim] remoteExec ["RES_fnc_citySupportChange",2];
					};
				};
			};
