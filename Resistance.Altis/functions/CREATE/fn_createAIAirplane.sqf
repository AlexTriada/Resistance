...
if (spawner getVariable _markerX != 2) then
	...
	if ([_typeVehX] call RES_fnc_vehAvailable) then
		{
		...
		for "_i" from 1 to _max do
			...
			{[_x,_markerX] call RES_fnc_NATOinit} forEach _vehCrew;
...
_ret = [_markerX,_size,_sideX,_frontierX] call RES_fnc_milBuildings;

{[_x] call RES_fnc_AIVEHinit} forEach (_ret select 1);
{[_x,_markerX] call RES_fnc_NATOinit} forEach (_ret select 2);
...
if (!_busy) then
	{
	private _runwaySpawnLocation = [_markerX] call RES_fnc_getRunwayTakeoffForAirportMarker;
	...
	if !(_runwaySpawnLocation isEqualTo []) then
		...
		while {(spawner getVariable _markerX != 2) and (_countX < 5)} do
			{
			_typeVehX = if (_sideX == Occupants) then {selectRandom (vehNATOAir select {[_x] call RES_fnc_vehAvailable})} else {selectRandom (vehCSATAir select {[_x] call RES_fnc_vehAvailable})};
...
if (_sideX == Occupants) then
	...
	_nul = [_veh] call RES_fnc_NATOcrate;
	_vehiclesX pushBack _veh;
	...
	_nul = [_veh] call RES_fnc_CSATcrate;
	_vehiclesX pushBack _veh;
	_veh call jn_fnc_logistics_addAction;
	};
...
if (!_busy) then
	{
	{
	_arrayVehAAF = if (_sideX == Occupants) then {vehNATOAttack select {[_x] call RES_fnc_vehAvailable}} else {vehCSATAttack select {[_x] call RES_fnc_vehAvailable}};
...
if (!(_x in staticsToSave)) then
	{
	if ((!([distanceSPWN-_size,1,_x,teamPlayer] call RES_fnc_distanceUnits))) then {deleteVehicle _x}
	};
} forEach _vehiclesX;
