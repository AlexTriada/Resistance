...
_nul = [_mrkOrigin,_mrkDestination,Invaders] spawn RES_fnc_artillery;
...
for "_i" from 1 to 3 do
	{
	_typeVehX = if (_i != 3) then {selectRandom (vehCSATAir select {[_x] call RES_fnc_vehAvailable})} else {selectRandom (_CSATTransportAir select {[_x] call RES_fnc_vehAvailable})};
	_timeOut = 0;
	...
	if (not(_typeVehX in _CSATTransportAir)) then
		...
		_typeGroup = [_typeVehX,Invaders] call RES_fnc_cargoSeats;
		_groupX = [_posOrigin, Invaders, _typeGroup] call RES_fnc_spawnGroup;
		{_x assignAsCargo _veh;_x moveInCargo _veh; _soldiers pushBack _x; [_x] call RES_fnc_NATOinit; _x setVariable ["originX",_mrkOrigin]} forEach units _groupX;
		_groups pushBack _groupX;
		//[_veh,"CSAT Air Transport"] spawn RES_fnc_inmuneConvoy;

		if (_typeVehX isKindOf "Plane") then {
			[_veh,_groupX,_mrkDestination,_mrkOrigin] spawn RES_fnc_airdrop;
		} else {
...
if (sidesX getVariable [_mrkDestination,sideUnknown] == Occupants) then {[[_posDestination,Occupants,"",false],"RES_fnc_patrolCA"] remoteExec ["RES_fnc_scheduler",2]};
...
for "_i" from 0 to round random 2 do
	...
	if ([vehCSATPlane] call RES_fnc_vehAvailable) then
		...
		_nul = [_mrkDestination,Invaders,_bombType] spawn RES_fnc_airstrike;
		sleep 30;
		};
	};
...
if ((({not (captive _x)} count _soldiers) < ({captive _x} count _soldiers)) or ({alive _x} count _soldiers < round (_solMax / 3)) or (time > _timeX)) then
	...
	["AttackAAF",[format ["%2 is making a punishment expedition to %1. They will kill everybody there. Defend the city at all costs",_nameDest,nameInvaders],format ["%1 Punishment",nameInvaders],_mrkDestination],getMarkerPos _mrkDestination,"SUCCEEDED"] call RES_fnc_taskUpdate;
	if ({(side _x == teamPlayer) and (_x distance _posDestination < _size * 2)} count allUnits >= {(side _x == Occupants) and (_x distance _posDestination < _size * 2)} count allUnits) then
		{
		if (sidesX getVariable [_mrkDestination,sideUnknown] == Occupants) then {[-15,15,_posDestination] remoteExec ["RES_fnc_citySupportChange",2]} else {[-5,15,_posDestination] remoteExec ["RES_fnc_citySupportChange",2]};
		[-5,0] remoteExec ["RES_fnc_prestige",2];
		{[-10,10,_x] remoteExec ["RES_fnc_citySupportChange",2]} forEach citiesX;
		...
		{
		if (sidesX getVariable [_mrkDestination,sideUnknown] == Occupants) then {[15,-5,_posDestination] remoteExec ["RES_fnc_citySupportChange",2]} else {[15,-15,_posDestination] remoteExec ["RES_fnc_citySupportChange",2]};
		{[10,-10,_x] remoteExec ["RES_fnc_citySupportChange",2]} forEach citiesX;
		};
	}
else
	{
	["AttackAAF",[format ["%2 is making a punishment expedition to %1. They will kill everybody there. Defend the city at all costs",_nameDest,nameInvaders],format ["%1 Punishment",nameInvaders],_mrkDestination],getMarkerPos _mrkDestination,"FAILED"] call RES_fnc_taskUpdate;
	//["AttackAAF", "FAILED",true] spawn BIS_fnc_taskSetState;
	[-20,-20,_posDestination] remoteExec ["RES_fnc_citySupportChange",2];
	{[-10,-10,_x] remoteExec ["RES_fnc_citySupportChange",2]} forEach citiesX;
