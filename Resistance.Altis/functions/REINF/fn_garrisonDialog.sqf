...
if ([_positionX,500] call RES_fnc_enemyNearCheck) exitWith {hint "You cannot manage this garrison while there are enemies nearby";_nul=CreateDialog "build_menu"};
...
if (_typeX == "rem") then
	...
	_hr = 0;
	{
	if (_x == staticCrewTeamPlayer) then {if (_outpostFIA) then {_costs = _costs + ([vehSDKLightArmed] call RES_fnc_vehiclePrice)} else {_costs = _costs + ([SDKMortar] call RES_fnc_vehiclePrice)}};
	_hr = _hr + 1;
	...
	hint format ["Info%1",[_nearX] call RES_fnc_garrisonInfo];
	closeDialog 0;
	...
	if (str (_display) != "no display") then
		...
		_ChildControl  ctrlSetTooltip format ["Cost: %1 €",(server getVariable staticCrewTeamPlayer) + ([SDKMortar] call RES_fnc_vehiclePrice)];
