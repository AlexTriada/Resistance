...
_road = [_positionX] call RES_fnc_findNearestGoodRoad;
_pos = position _road;
...
if ((spawner getVariable _markerX != 2) and !(sidesX getVariable [_markerX,sideUnknown] == teamPlayer)) then
	...
	_truckCreated = true;
	if (_sideX == Occupants) then {[_truckX] call RES_fnc_NATOcrate} else {[_truckX] call RES_fnc_CSATcrate};
	...
	_truckX addEventHandler ["GetIn", {
		...
		[getPosASL _vehicle, _owningSide, "", false] spawn RES_fnc_patrolCA;
	...
	if (dateToNumber date > _dateLimitNum) then
		{
		["LOG",[format ["We've spotted an Ammotruck in an %1. Go there and destroy or steal it before %2.",_nameDest,_displayTime],"Steal or Destroy Ammotruck",_markerX],_positionX,"FAILED","rearm"] call RES_fnc_taskUpdate;
	...
	if ((not alive _truckX) or (call _fnc_truckReturnedToBase)) then
		{
			...
			["LOG",[format ["We've spotted an Ammotruck in an %1. Go there and destroy or steal it before %2.",_nameDest,_displayTime],"Steal or Destroy Ammotruck",_markerX],_positionX,"SUCCEEDED","rearm"] call RES_fnc_taskUpdate;
...
if (_truckCreated) then
	...
	deleteMarker _mrk;
	waitUntil {sleep 1; !([300,1,_truckX,teamPlayer] call RES_fnc_distanceUnits)};
