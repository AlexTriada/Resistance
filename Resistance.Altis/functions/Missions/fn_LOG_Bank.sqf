...
_nameDest = [_city] call RES_fnc_localizar;
...
[_truckX,"Mission Vehicle"] spawn RES_fnc_inmuneConvoy;
...
for "_i" from 1 to 4 do
	{
	_groupX = if (_difficultX) then {[_positionX,Occupants, groupsNATOSentry] call RES_fnc_spawnGroup} else {[_positionX,Occupants, groupsNATOGen] call RES_fnc_spawnGroup};
	sleep 1;
...
if ((dateToNumber date > _dateLimitNum) or (!alive _truckX)) then
	...
	[[_positionX,Occupants,"",true],"RES_fnc_patrolCA"] remoteExec ["RES_fnc_scheduler",2];
	[10*_bonus,-20*_bonus,_markerX] remoteExec ["RES_fnc_citySupportChange",2];
	...
	} forEach ([distanceSPWN,0,_positionX,teamPlayer] call RES_fnc_distanceUnits);
	_exit = false;
	while {(_countX > 0) or (_truckX distance _positionX < 7) and (alive _truckX) and (dateToNumber date < _dateLimitNum)} do
		...
		if (_countX > 0) then
			{
			_countX = 120*_bonus;//120
			if (_truckX distance _positionX > 6) then {{[petros,"hint","Don't get the truck far from the bank or count will restart"] remoteExec ["RES_fnc_commsMP",_x]} forEach ([200,0,_truckX,teamPlayer] call RES_fnc_distanceUnits)};
			...
			if (alive _truckX) then
				{
				{if (isPlayer _x) then {[petros,"hint","Drive the Truck back to base to finish this mission"] remoteExec ["RES_fnc_commsMP",_x]}} forEach ([80,0,_truckX,teamPlayer] call RES_fnc_distanceUnits);
				_exit = true;
				};
			//waitUntil {sleep 1; (!alive _truckX) or (_truckX distance _positionX > 7) or (dateToNumber date < _dateLimitNum)};
			};
...
if ((_truckX distance _posbase < 50) and (dateToNumber date < _dateLimitNum)) then
	{
	["LOG",[format ["We know Gendarmes is guarding a large amount of money in the bank of %1. Take this truck and go there before %2, hold the truck close to tha bank's main entrance for 2 minutes and the money will be transferred to the truck. Bring it back to HQ and the money will be ours.",_nameDest,_displayTime],"Bank Robbery",_mrkFinal],_positionX,"SUCCEEDED","Interact"] call RES_fnc_taskUpdate;
	[0,5000*_bonus] remoteExec ["RES_fnc_resourcesFIA",2];
	[10*_bonus,0] remoteExec ["RES_fnc_prestige",2];
	[1800*_bonus] remoteExec ["RES_fnc_timingCA",2];
	{if (_x distance _truckX < 500) then {[10*_bonus,_x] call RES_fnc_playerScoreAdd}} forEach (allPlayers - (entities "HeadlessClient_F"));
	[5*_bonus,theBoss] call RES_fnc_playerScoreAdd;
	waitUntil {sleep 1; speed _truckX == 0};

	[_truckX] call RES_fnc_empty;
	};
if (!alive _truckX) then
	{
	["LOG",[format ["We know Gendarmes is guarding a large amount of money in the bank of %1. Take this truck and go there before %2, hold the truck close to tha bank's main entrance for 2 minutes and the money will be transferred to the truck. Bring it back to HQ and the money will be ours.",_nameDest,_displayTime],"Bank Robbery",_mrkFinal],_positionX,"FAILED","Interact"] call RES_fnc_taskUpdate;
...
waitUntil {sleep 1; !([distanceSPWN,1,_positionX,teamPlayer] call RES_fnc_distanceUnits)};
