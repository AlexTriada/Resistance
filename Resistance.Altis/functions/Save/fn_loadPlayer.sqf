...
if (isMultiplayer && !isServer) exitwith {
	[_playerId, _unit] remoteExec ["RES_fnc_loadPlayer", 2];
};

waitUntil {(!isNil "initVar")};

if ([_playerId] call RES_fnc_playerHasSave) then {
...
if (isMultiplayer && side _unit == teamPlayer) then
{
	...
	if ([_unit] call RES_fnc_isMember) then
	{
		...
		[_unit, _rank] remoteExec ["RES_fnc_ranksMP"];
	...
	[_unit, [_playerId, "personalGarage"] call fn_RetrievePlayerStat] call RES_fnc_setPersonalGarage;
};
diag_log format ["%1: [Antistasi] | INFO | Personal player stats loaded.",servertime];

[] remoteExec ["RES_fnc_statistics", _unit];
