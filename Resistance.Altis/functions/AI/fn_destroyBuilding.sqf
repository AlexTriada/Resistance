...
while {true} do
	{
	if !([_engineerX] call RES_fnc_canFight) exitWith {};
...
if !([_engineerX] call RES_fnc_canFight) exitWith
	...
	_engineerX call RES_fnc_recallGroup;
...
if (_engineerX != leader _engineerX) then {_engineerX doMove (getPos (leader _engineerX))} else {_engineerX call RES_fnc_recallGroup};
