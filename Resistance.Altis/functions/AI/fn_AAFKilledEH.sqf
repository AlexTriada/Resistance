...
//if (_killer isEqualType "") then {diag_log format ["Antistasi error in RES_fnc_AAFKilledEH, params: %1",_this]};
if (side (group _killer) == teamPlayer) then
	{
	if (isPlayer _killer) then
		{
		[1,_killer] call RES_fnc_playerScoreAdd;
	...
	if (count weapons _victim < 1 && !(_victim getVariable ["isAnimal", false])) then
		...
		if (_sideX == Occupants) then
			{
			[0,-2,getPos _victim] remoteExec ["RES_fnc_citySupportChange",2];
			[1,0] remoteExec ["RES_fnc_prestige",2];
			}
		else
			{
			[0,1] remoteExec ["RES_fnc_prestige",2];
			};
		}
	else
		{
		[-1,1,getPos _victim] remoteExec ["RES_fnc_citySupportChange",2];
		if (_sideX == Occupants) then
			{
			[0.1,0] remoteExec ["RES_fnc_prestige",2];
			}
		else
			{
	...
	if (_sideX == Occupants) then
		{
		[-0.25,0,getPos _victim] remoteExec ["RES_fnc_citySupportChange",2];
		}
	else
		{
		[0.25,0,getPos _victim] remoteExec ["RES_fnc_citySupportChange",2];
...
if (_markerX != "") then
	...
	if (sidesX getVariable [_markerX,sideUnknown] == _sideX) then
		{
		[typeOf _victim,_sideX,_markerX,-1] remoteExec ["RES_fnc_garrisonUpdate",2];
		if (_garrisoned) then {[_markerX,_sideX] remoteExec ["RES_fnc_zoneCheck",2]};
		};
	};
[_groupX,_killer] spawn RES_fnc_AIreactOnKill;
